...
Produces N+1 keyframe images for N clips:
  ...
  Frames 1..N  : Expression-transition frames — generated by Gemini Flash Image.
                 Each one edits the Frame 0 master reference and changes ONLY the
                 expression to match the end_emotion of that clip (no daisy-chain).
                 Clip boundaries still align: last pixel of clip i = first of clip i+1.
...
FALLBACK for Frames 1..N:
  If Gemini Image edit fails → reuse master reference pixels (seamless cut still works,
  just expression won't change at that boundary); keyframe is flagged for regen.
"""
...
def generate_keyframes(
    ...
    for i, clip in enumerate(clips):
        ...
        if not is_valid:
            ...
            next_frame.validation_issues = list(next_frame.validation_issues or []) + issues
...
def _generate_transition_frame(
    ...
    if result_b64 is None:
        # Fallback: reuse master reference frame (frame_0).
        # The cut is still seamless but the expression at this boundary
        # won't change — this is a visible quality issue.
        logger.warning(
            "Phase 3: Gemini Image returned no image for Frame %d "
            "(Clip %d end-emotion: %s) — falling back to master reference. "
            "OPERATOR SHOULD REGEN THIS KEYFRAME.",
        ...
        return KeyFrame(
            ...
            mime_type="image/jpeg",
            description=f"Clip {clip.clip_number} end-frame — FALLBACK (expression not applied)",
            approved=False,
            validation_issues=[
                "Gemini Image returned no output — expression not changed. Please regenerate this keyframe.",
            ],
        )
